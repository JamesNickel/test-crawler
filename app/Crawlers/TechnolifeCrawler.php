...
class TechnolifeCrawler extends SourceCrawler
{
    public function crawl(int $crawlId): void
    {
        ...
        $topCategories = $dom->filter('header-module__firstLevelMenuLi___H1R-d > a')->each(function ($node) {
            return [
                'name' => trim($node->text()),
    ...
    private function crawlCategoryPage(int $categoryId, string $url, int $crawlId): void
    {
        ...
        while (true) {
            ...
            $hasProducts = $dom->filter('article > section')->count() > 0;
            if (!$hasProducts) break;

            $dom->filter('article > section')->each(function ($node) use ($categoryId, $crawlId) {
                $product = [
                    'name'        => $node->filter('h2.overflow-hidden')->text(),
                    'url'         => $this->makeAbsoluteUrl($node->filter('a[target="_blank"]')->attr('href')),
                    'external_id' => '',
                    'price'       => $this->parsePrice($node->filter('p.text-gray-800')->text()),
                    'score'       => $this->parseFloat($node->filter('p.text-gray-600')->text() ?? 0),
                    'score_count' => 0,
                ];
